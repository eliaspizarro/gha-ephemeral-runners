...
```mermaid
graph LR
    A[Inicio Contenedor] --> B[entrypoint.sh]
    B --> C[Configura runner]
    C --> D{RUNNER_COMMAND?}
    
    D -->|Sí| E[RUNNER_COMMAND<br/>Sobrescribe CMD]
    D -->|No| F[CMD por Defecto<br/>Runner.Listener run]
    
    E --> G[Procesos definidos<br/>Runner.Listener, filtrado]
